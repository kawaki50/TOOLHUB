...
const NewArrivals = () => {
    ...
    useEffect(() => {
        const fetchProducts = async () => {
            try {
                const response = await fetch('http://localhost/TOOLHUB/back-end/api/newArrivals.php');
    ...
    return (
        ...
        <div className="seller container">
            <h2 style={{textAlign: 'start'}}>New Arrivals</h2>
            <br />
            <div className="best-seller">
                {products.map((product) => (
                    <div key={product.id_produits} className="best-p1">
                        <img src={`/src/assets/img/new-arrivals/${product.image}`} alt={product.nom} style={{marginBottom: '20px'}}/>
